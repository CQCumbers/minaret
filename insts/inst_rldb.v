...
module mfi_inst_rldb (
  ...
  // RLDB instruction
  wire [31:0] addr = mfi_src1_rdata + mfi_src2_rdata;
  wire [7:0] result = mfi_mem_rdata >> (8*(addr-spec_mem_addr));
  assign spec_valid = mfi_valid && !inst_padding && inst_opcode == 8'b 01001010;
  assign spec_src1_addr = inst_src1;
  assign spec_src2_addr = inst_src2;
  assign spec_dest_addr = inst_dest;
  assign spec_mem_addr = addr & ~(1-1);
