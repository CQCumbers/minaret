...
module mfi_inst_rcall (
  ...
  // RCALL instruction
  wire [31:0] result = mfi_src1_rdata + 4 * inst_imms;
  wire [31:0] addr = mfi_src3_rdata - 4;
  assign spec_valid = mfi_valid && !inst_padding && inst_opcode == 8'b 00110001;
  assign spec_src1_addr = inst_src1;
  assign spec_src3_addr = 15;
  assign spec_dest_addr = 15;
  assign spec_dest_wdata = addr & ~3;
  assign spec_mem_addr = addr & ~3;
  assign spec_mem_wmask = 4'b1111 << (addr-spec_mem_addr);
  assign spec_mem_wdata = (mfi_pc_rdata + 4) << (8*(addr-spec_mem_addr));
